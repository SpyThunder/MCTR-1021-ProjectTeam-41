...
from HHO import hho_optimize
from Surrogate import surrogate_optimize
from PID import simulate_and_cost, visualize_pid, straight_traffic_factory

# ---------------------------
# Utilities (fixed & updated)
# ---------------------------
def plot_convergence(history_ga, history_sa=None, history_pso=None, history_hho=None,
                     history_ml=None,
    ...
    # HHO history (list of tuples (it, best_cost, best_gains))
    if history_hho is not None:
        gens_hho = [x[0] for x in history_hho]
        bests_hho = [x[1] for x in history_hho]
        L = min(len(history_ga["gen"]), len(bests_hho))
        plt.plot(history_ga["gen"][:L], bests_hho[:L], label="HHO best", linestyle="-.", linewidth=1.5)

    # ML surrogate history (list of tuples (round, best_cost, best_gains) OR (round, best_cost, best_gains, dataset_size))
    if history_ml is not None:
        bests_ml = [x[1] for x in history_ml]
        L = min(len(history_ga["gen"]), len(bests_ml))
        plt.plot(history_ga["gen"][:L], bests_ml[:L], label="ML (Surrogate) best", linestyle=(0, (3, 1, 1, 1)), linewidth=1.5)
...
def run_case_study(case_name, path_params=(40, 4, 300), other_cars_fn=None,
                   ga_params=None, sa_params=None, pso_params=None, hho_params=None, ml_params=None,
                   compare_to_sa=True, compare_to_pso=True, compare_to_hho=True, compare_to_ml=True,
                   ga_runs=5, sa_runs=5, pso_runs=5, hho_runs=5, ml_runs=5):
    ...
    hho_multi = None
    if compare_to_hho:
        print(f"\n[HHO] Running {hho_runs} times...")
        t0 = time.time()
        hho_multi = run_hho_multiple(hho_runs, hho_params or {}, other_cars_fn)
        t_hho = time.time() - t0

        print("\n[HHO] SUMMARY")
        print(f"  Mean cost: {hho_multi['mean']:.4f}")
        print(f"  Std cost : {hho_multi['std']:.4f}")
        print(f"  Best cost: {min(hho_multi['best_costs']):.4f}")
        print(f"  Best gains: {hho_multi['best_overall']}")
        print(f"  Time: {t_hho:.2f}s")

    # ---------------------------
    # ML (SURROGATE) MULTI-RUN
    # ---------------------------
    ml_multi = None
    if compare_to_ml:
        print(f"\n[ML] Running {ml_runs} times (Surrogate regression)...")
        t0 = time.time()
        ml_multi = run_ml_multiple(ml_runs, ml_params or {}, other_cars_fn)
        t_ml = time.time() - t0

        print("\n[ML] SUMMARY")
        print(f"  Mean cost: {ml_multi['mean']:.4f}")
        print(f"  Std cost : {ml_multi['std']:.4f}")
        print(f"  Best cost: {min(ml_multi['best_costs']):.4f}")
        print(f"  Best gains: {ml_multi['best_overall']}")
        print(f"  Time: {t_ml:.2f}s")
    ...
    history_hho = hho_multi["histories"][0] if hho_multi else None
    history_ml = ml_multi["histories"][0] if ml_multi else None

    plot_convergence(history_ga, history_sa, history_pso, history_hho, history_ml, title=f"Convergence - {case_name}")
    ...
    if hho_multi:
        gains_list.append(hho_multi["best_overall"])
        labels.append("HHO best over runs")

    if ml_multi:
        gains_list.append(ml_multi["best_overall"])
        labels.append("ML (Surrogate) best over runs")

    plot_cte_comparison(gains_list, labels)

    return {"ga_multi": ga_multi, "sa_multi": sa_multi, "pso_multi": pso_multi, "hho_multi": hho_multi, "ml_multi": ml_multi}
...
def default_suite():
    ...
    results["case_collision"] = run_case_study(
        ...
        ml_params={
            # Surrogate is budgeted in 'rounds' rather than iterations.
            # For a similar wall-clock budget, keep rounds modest and proposals moderate.
            "initial_samples": 60,
            "rounds": 12,
            "proposals_per_round": 3000,
            "topk_validate": 8,
            "ridge_lambda": 1e-6,
        },
        compare_to_pso=True,
...
def run_hho_multiple(times, hho_params, other_cars_fn=None):
    best_costs = []
    best_gains = []
    histories = []

    for i in range(times):
        print(f"\n[HHO] Multi-run {i+1}/{times}")
        mapped = dict(
            iterations=hho_params.get("iterations", 100),
            population_size=hho_params.get("population_size", 20),
            bounds=((0.0, 100.0), (0.0, 10.0), (0.0, 10.0)),
            evaluate_fn=simulate_and_cost,
            other_cars_fn=other_cars_fn,
            seed=i,
...
def run_ml_multiple(times, ml_params, other_cars_fn=None):
    """Run the supervised surrogate optimizer multiple times.

    Note: surrogate_optimize has a different internal notion of 'iteration' (rounds + simulator calls).
    For plotting, we use its per-round best-so-far history.
    """
    best_costs = []
    best_gains = []
    histories = []
    ...
    for i in range(times):
        print(f"\n[ML] Multi-run {i+1}/{times}")
        p = dict(
            bounds=((0.0, 100.0), (0.0, 10.0), (0.0, 10.0)),
            evaluate_fn=simulate_and_cost,
            other_cars_fn=other_cars_fn,
            seed=i,
            initial_samples=ml_params.get("initial_samples", 60),
            rounds=ml_params.get("rounds", 12),
            proposals_per_round=ml_params.get("proposals_per_round", 3000),
            topk_validate=ml_params.get("topk_validate", 8),
            ridge_lambda=ml_params.get("ridge_lambda", 1e-6),
            verbose_every=ml_params.get("verbose_every", 0),
            visualize_every_round=None,
            visualize_blocking=False,
        )

        out = surrogate_optimize(**p)
        best_costs.append(out["best_cost"])
        best_gains.append(out["best_gains"])
        histories.append(out["history"])
...
def plot_gaussian_comparison(ga_mean, ga_std,
                             sa_mean=None, sa_std=None,
                             pso_mean=None, pso_std=None,
                             hho_mean=None, hho_std=None,
                             ml_mean=None, ml_std=None):
    """
    ...
    # HHO distribution (if provided)
    if hho_mean is not None and hho_std is not None:
        plot_gaussian(hho_mean, hho_std, label="HHO Distribution", color="purple")

    # ML distribution (if provided)
    if ml_mean is not None and ml_std is not None:
        plot_gaussian(ml_mean, ml_std, label="ML (Surrogate) Distribution", color="orange")
...
if __name__ == "__main__":
    ...
    for k, v in results.items():
        ...
        if v.get("pso_multi") is not None:
            pso = v["pso_multi"]
            pso_mean = pso["mean"]
            pso_std = pso["std"]
        ...
        if v.get("hho_multi") is not None:
            hho = v["hho_multi"]
            hho_mean = hho["mean"]
            hho_std = hho["std"]
            print(f"     HHO mean={hho['mean']:.6f} std={hho['std']:.6f} best={min(hho['best_costs']):.6f}")
            print(f"     HHO best gains: {hho['best_overall']}")
        else:
            hho_mean = None
            hho_std = None

        if v.get("ml_multi") is not None:
            ml = v["ml_multi"]
            ml_mean = ml["mean"]
            ml_std = ml["std"]
            print(f"     ML mean={ml['mean']:.6f} std={ml['std']:.6f} best={min(ml['best_costs']):.6f}")
            print(f"     ML best gains: {ml['best_overall']}")
        else:
            ml_mean = None
            ml_std = None
    ...
    plot_gaussian_comparison(ga_mean, ga_std, sa_mean, sa_std, pso_mean, pso_std, hho_mean, hho_std, ml_mean, ml_std)
